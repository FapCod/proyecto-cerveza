...
function QuitarCerveza() {
    ...
    const editarCaja = async (id)=>{
        ...
        try {
            ...
            setBandera(data.data().stock/12)
            let valorUnidad= cantidadCaja*12
            setCantidadUni(valorUnidad)
            setId(id)
        } catch (error) {
            console.error(error)
        }
    ...
    const editarCervezaCaja = async (e)=>{
        ...
        if (isNaN(cantidadCaja)===true) {
            ...
            let valorEnviarCantidad =cantidadCaja*12
            const cervezaActual = {
                stock: (bandera*12) - valorEnviarCantidad
            }
            try {
    ...
    const editarCervezaUni = async (e)=>{
        ...
        if (isNaN(cantidadUni)===true) {
            ...
            setError(null)
            const cervezaActual = {
                stock: bandera-cantidadUni
            }
            try {
                await DBstore.collection('cerveza').doc(id).set(cervezaActual)
                alert("Cerveza Quitada")
                getCerveza()
            } catch (error) {
                console.error(error)
            }
    ...
    return (
        ...
        <form onSubmit={(e)=>{modoEdicion ? editarCervezaCaja(e) : agregarCervezaPorCaja(e)}}>
            <div>
            <div className="mb-3">
                <label  className="form-label ">Quitar por Cajas</label>
                <input   required onChange={(e) => { setCantidadCaja(e.target.value) }}  type="number" className="form-control" value={cantidadCaja}  placeholder="0"/>
            </div>
                    {
                        modoEdicion ?
                        (<button type="submit" className="btn btn-danger ">Eliminar Caja</button>)
                        :
        ...
        <form onSubmit={(e)=>{modoEdicion ? editarCervezaUni(e) : agregarCervezaPorUni(e)}}>
            <div className="ml-3">
            <div className="mb-3">
                <label    className="form-label">Quitar por Unidades</label>
                <input onChange={(e) => { setCantidadUni(e.target.value) }} type="number" className="form-control"  placeholder="0" value={cantidadUni}/>
            </div>
                    {
                        modoEdicion ?
                        (<button type="submit" className="btn btn-danger ">Eliminar Unidad</button>)
                        :
                        (<button type="submit" className="btn btn-danger ">Eliminar</button>)
                    }
